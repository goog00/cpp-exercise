...
#define MAX_ROWS 8
#define MAX_COLS 8
#define STRIDE 64  // AMX 要求 64 字节对齐
...
void init_tile_config(__tile_config* tileinfo) {
    ...
    tileinfo->rows[2] = MAX_ROWS;  // Tile 2: src1 (8x8 INT8)
    tileinfo->colsb[2] = MAX_COLS; // 8 bytes

    tileinfo->rows[3] = MAX_ROWS;  // Tile 3: src2 (8x8 INT8)
    tileinfo->colsb[3] = MAX_COLS; // 8 bytes

    tileinfo->rows[1] = MAX_ROWS;  // Tile 1: dst (8x8 INT32)
    tileinfo->colsb[1] = MAX_COLS * sizeof(int32_t); // 32 bytes
